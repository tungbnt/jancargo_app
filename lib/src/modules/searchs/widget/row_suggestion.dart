  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: dto != null && dto!.isNotEmpty
            ? dto
                !.map((e) =>
                    columnSuggest(e.name!, e.name == 'Giày hiệu' ? false : true,e.categoryId.toString(),),)
                .toList()
            : suggestions
                .map((e) => columnSuggest(e, e == 'Giày hiệu' ? false : true,"23140",),)
                .toList(),
      ),
    );
  }

  Widget columnSuggest(String text, bool isDivider,String code) => GestureDetector(
        onTap: () {
          getCurrentIndex?.call(text);
          AppManager.appSession.saveCodeCategory(code);
          AppManager.appSession.saveNameCategory(text);
        },
        child: SizedBox(
