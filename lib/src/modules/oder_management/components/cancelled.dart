...
class Cancelled extends StatefulWidget {
  const Cancelled({super.key, required this.cubit});
...
class _CancelledState extends State<Cancelled> {
  ...
  void _getOder(){
    return widget.cubit.cancelled();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<OderManagementCubit, OderManagementState>(
      bloc: widget.cubit,
      buildWhen: (prv,state)=> state is OderManagementCancelledSuccess || state is OderManagementEmpty || state is OderManagementLoading,
      builder: (context, state) {
        ...
        if(state is OderManagementCancelledSuccess) {
          return Padding(
