...
class SuccessDelivery extends StatefulWidget {
  const SuccessDelivery({super.key, required this.cubit});
...
class _SuccessDeliveryState extends State<SuccessDelivery> {
  ...
  void _getOder(){
    return widget.cubit.successDelivery();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<OderManagementCubit, OderManagementState>(
      bloc: widget.cubit,
      buildWhen: (prv,state)=> state is OderManagementSuccessDelivery || state is OderManagementEmpty || state is OderManagementLoading,
      builder: (context, state) {
        ...
        if(state is OderManagementSuccessDelivery) {
          return Padding(
