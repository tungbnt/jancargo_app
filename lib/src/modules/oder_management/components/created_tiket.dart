...
class CreatedTicket extends StatefulWidget {
  const CreatedTicket({super.key, required this.cubit});

  final OderManagementCubit cubit;

  @override
  State<CreatedTicket> createState() => _CreatedTicketState();
}

class _CreatedTicketState extends State<CreatedTicket> {
  @override
  void initState() {
    super.initState();
    _getOder();
  ...
  void _getOder(){
    return widget.cubit.createdTicket();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<OderManagementCubit, OderManagementState>(
      bloc: widget.cubit,
      buildWhen: (prv,state)=> state is OderManagementCreatedTicketSuccess || state is OderManagementEmpty || state is OderManagementLoading,
      builder: (context, state) {
        ...
        if(state is OderManagementCreatedTicketSuccess) {
          return Padding(
