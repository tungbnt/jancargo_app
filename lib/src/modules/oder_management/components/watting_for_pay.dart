...
class WattingForPay extends StatefulWidget {
  const WattingForPay({super.key, required this.cubit});

  final OderManagementCubit cubit;

  @override
  State<WattingForPay> createState() => _WattingForPayState();
}

class _WattingForPayState extends State<WattingForPay> {
  @override
  void initState() {
    super.initState();
    _getOder();
  ...
  void _getOder(){
    return widget.cubit.waitForPay();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<OderManagementCubit, OderManagementState>(
      bloc: widget.cubit,
      buildWhen: (prv,state)=> state is OderManagementWaitForPaySuccess || state is OderManagementEmpty || state is OderManagementLoading,
