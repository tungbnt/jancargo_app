...
class Processing extends StatefulWidget {
  const Processing({super.key, required this.cubit});
...
class _ProcessingState extends State<Processing> {
  ...
  void _getOder(){
    return widget.cubit.processing();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<OderManagementCubit, OderManagementState>(
      bloc: widget.cubit,
      buildWhen: (prv,state)=> state is OderManagementProcessingSuccess || state is OderManagementEmpty || state is OderManagementLoading,
      builder: (context, state) {
        ...
        if(state is OderManagementProcessingSuccess) {
          return Padding(
