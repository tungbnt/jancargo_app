...
class OderAll extends StatefulWidget {
  const OderAll({super.key, required this.cubit, this.step});

  final OderManagementCubit cubit;
  final String? step;

  @override
  State<OderAll> createState() => _OderAllState();
}

class _OderAllState extends State<OderAll> {
  @override
  void initState() {
    super.initState();
    _getOder();
  ...
  void _getOder(){
    return widget.cubit.oderAll();
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<OderManagementCubit, OderManagementState>(
      bloc: widget.cubit,
      buildWhen: (prv,state)=> state is OderManagementAllSuccess || state is OderManagementEmpty || state is OderManagementLoading,
      builder: (context, state) {
        ...
        if(state is OderManagementAllSuccess) {
          return Padding(
