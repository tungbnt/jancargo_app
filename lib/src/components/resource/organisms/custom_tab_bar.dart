...
class CustomTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// An optional callback that's called before the [TabBar] is tapped.
  final ValueChanged<int>? onBeforeTap;
  ...
  Size get preferredSize {
    ...
    return maxHeight == _kTabHeight ? const Size.fromHeight(_kTabHeight) : Size.fromHeight(maxHeight + indicatorWeight);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isScrollable) {
      ...
      if (widget.indicator?.borderRadius != null)
        tabBar = ClipRRect(
          borderRadius: widget.indicator!.borderRadius as BorderRadiusGeometry,
          child: tabBar,
        );
...
/// [slivers] list and the length of the [TabBar.tabs] list.
///
...
class FlexibleTabBarView extends StatefulWidget {
  ...
  final ValueSetter<PageController>? onPageControllerChanged;
  ...
  /// The estimated size of displayed [TabBarView]/pages.
  ///
  ...
  /// [FlexibleTabBarView] size animation. This can lead to a behaviour
  /// when after changing the page, [FlexibleTabBarView] will first shrink to 0.0
  /// and then animate to the size of the page.
  ///
  /// For example: If there is certainty that most pages displayed by [FlexibleTabBarView]
  ...
  @override
  State<FlexibleTabBarView> createState() => _FlexibleTabBarViewState();
}

class _FlexibleTabBarViewState extends State<FlexibleTabBarView> with AutomaticKeepAliveClientMixin {
  TabController? _controller;
  PageController? __pageController;
  PageController get _pageController => __pageController!;
  ...
  /// If the [FlexibleTabBarView] is rebuilt with a new tab controller,
  /// the caller should dispose the old one.
  /// In that case the old controller's animation will be
  /// null and should not be accessed.
  bool get _controllerIsValid => _controller?.animation != null;
  ...
  void initState() {
    super.initState();
    _heights = _generateEstimatedSizes();

    final SliverChildDelegate childrenDelegate = widget._childrenDelegate;
    if (childrenDelegate is SliverChildListDelegate) {
      _updateChildren(childrenDelegate.children);
    }
  }

  List<double> _generateEstimatedSizes() {
    return List.generate(
      _estimateChildCount(),
      (index) => widget.estimatedPageSize,
    );
  }

  int _estimateChildCount() {
    final SliverChildDelegate childrenDelegate = widget._childrenDelegate;

    if (childrenDelegate is SliverChildBuilderDelegate) {
      return childrenDelegate.childCount ?? 0;
    } else if (childrenDelegate is SliverChildListDelegate) {
      return childrenDelegate.children.length;
    } else {
      return childrenDelegate.estimatedChildCount ?? 0;
    }
  }
  ...
  void didChangeDependencies() {
    ...
    __pageController?.removeListener(_updatePage);
    ...
    // }
    widget.onPageControllerChanged?.call(_pageController);
  ...
  void didUpdateWidget(FlexibleTabBarView oldWidget) {
    ...
    final SliverChildDelegate childrenDelegate = widget._childrenDelegate;
    final SliverChildDelegate oldChildrenDelegate = oldWidget._childrenDelegate;

    assert(childrenDelegate.runtimeType == oldChildrenDelegate.runtimeType);

    if (oldChildrenDelegate is SliverChildListDelegate && childrenDelegate is SliverChildListDelegate) {
      if (childrenDelegate.children != oldChildrenDelegate.children && _warpUnderwayCount == 0) {
        _updateChildren(childrenDelegate.children);
      }
      if (childrenDelegate.children.length != oldChildrenDelegate.children.length) {
        _reinitializeSizes();
      }
    } else if (oldChildrenDelegate is SliverChildBuilderDelegate && childrenDelegate is SliverChildBuilderDelegate) {
      _reinitializeSizes();
    }
  }
  ...
  void _reinitializeSizes() {
    ...
    int previousIndex = _previousIndex!;

    final currentPageHeight = _heights[currentIndex];

    _heights = _generateEstimatedSizes();

    if (currentIndex >= _heights.length) {
      final differenceFromPreviousToCurrent = previousIndex - currentIndex;
      currentIndex = _heights.length - 1;
      previousIndex = (currentIndex + differenceFromPreviousToCurrent).clamp(
        0,
        _heights.length - 1,
      );
    }

    previousIndex = previousIndex.clamp(0, _heights.length - 1);
    _heights[currentIndex] = currentPageHeight;
  ...
  Future<void> _warpToCurrentIndex() async {
    ...
    setState(() {
      ...
      final SliverChildDelegate childrenDelegate = widget._childrenDelegate;
      if (childrenDelegate is SliverChildListDelegate && childrenDelegate.children != _children) {
        _updateChildren(childrenDelegate.children);
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    if (childrenDelegate is SliverChildBuilderDelegate) {
      pageView = PageView.builder(
        ...
        itemCount: childrenDelegate.childCount,
        itemBuilder: (context, index) {
          return _OverflowView(
            viewIndex: index,
            onSizeChange: _handleSizeChange,
            child: KeyedSubtree.wrap(
              childrenDelegate.builder(context, index)!,
              index,
            ),
          );
        },
  ...
  bool _debugScheduleCheckHasValidChildrenCount() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((Duration duration) {
      ...
      assert(() {
        final int estimatedChildCount = _estimateChildCount();
        if (_controller!.length != estimatedChildCount) {
          throw FlutterError(
            "Controller's length property (${_controller!.length}) does not match the "
            "number of children ($estimatedChildCount) present in TabBarView's children property.",
          );
        }
        return true;
      }());
  ...
  void _handleSizeChange(int index, Size value) {
    setState(() => _heights[index] = value.height);
  }
...
class _OverflowView extends StatelessWidget {
  const _OverflowView({
    Key? key,
  ...
  final int viewIndex;
  final void Function(int index, Size value) onSizeChange;
  final Widget child;

  void _handleSizeChange(Size value) {
    onSizeChange(viewIndex, value);
  }
